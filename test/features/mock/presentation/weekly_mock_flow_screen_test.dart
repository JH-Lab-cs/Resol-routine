...
import 'package:resol_routine/features/mock_exam/presentation/weekly_mock_flow_screen.dart';
...
void main() {
  group('WeeklyMockFlowScreen', () {
    testWidgets('home weekly card opens weekly mock flow', (
      WidgetTester tester,
    ) async {
      ...
      expect(find.text('주간 모의고사'), findsOneWidget);
      await tester.tap(find.text('주간 모의고사'));
      await tester.pumpAndSettle();

      expect(find.text('주간 모의고사'), findsAtLeastNWidgets(1));
      expect(find.text('시작하기'), findsOneWidget);

      await tester.tap(find.text('시작하기'));
      await tester.pumpAndSettle();

      expect(find.text('문제 1 / 20'), findsOneWidget);
    });
    ...
    testWidgets('resumes from next unanswered question', (
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [appDatabaseProvider.overrideWithValue(database)],
          child: MaterialApp(
            home: WeeklyMockFlowScreen(track: 'M3', nowLocal: nowLocal),
          ),
        ),
      );
      await tester.pumpAndSettle();
      ...
      expect(find.text('주간 모의고사'), findsOneWidget);
      expect(find.text('문제 2 / 20'), findsOneWidget);
    });
    ...
    testWidgets('shows completion screen when all attempts already exist', (
      ...
      expect(find.text('주간 모의고사 완료'), findsOneWidget);
      expect(find.textContaining('20문제'), findsOneWidget);
    });
  });
}
