...
def evaluate_content_calibration(
    *,
    ...
    if (
        calibrated_level.value
        not in CONTENT_CALIBRATION_ACCEPTABLE_LEVELS_BY_TRACK[unit.track.value]
    ):
    ...
    if discourse < _discourse_minimum_for_track(unit.track, unit.skill):
        fail_reasons.append("discourse_complexity_below_track_baseline")
    if structure < _structure_minimum_for_track(unit.track, unit.skill):
    ...
    if (
        not passed
        and unit.track.value in CONTENT_CALIBRATION_ALLOWED_WARNING_TRACKS
...
def is_calibration_publish_blocked(*, track: Track, result: ContentCalibrationResult) -> bool:
...
def _apply_type_tag_specific_rules(
    *,
    ...
    if type_tag == ContentTypeTag.R_INSERTION:
        ...
        if track in {Track.H1, Track.H2, Track.H3} and inference < 35:
...
def _discourse_minimum_for_track(track: Track, skill: Skill) -> int:
    base = {
        Track.M3: 18,
        Track.H1: 24,
        Track.H2: 34,
...
def _structure_minimum_for_track(track: Track, skill: Skill) -> int:
    base = {
        Track.M3: 20,
        Track.H1: 28,
        Track.H2: 38,
