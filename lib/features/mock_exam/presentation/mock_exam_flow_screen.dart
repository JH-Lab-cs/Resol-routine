...
enum MockExamFlowExitAction { home, wrongNotes }

class MockExamFlowConfig {
  const MockExamFlowConfig({
    required this.type,
    required this.title,
    required this.plan,
    required this.startDescription,
    required this.orderDescription,
    required this.insufficientDescription,
  });

  final MockExamType type;
  final String title;
  final MockExamQuestionPlan plan;
  final String startDescription;
  final String orderDescription;
  final String insufficientDescription;
}

const MockExamFlowConfig weeklyMockExamFlowConfig = MockExamFlowConfig(
  type: MockExamType.weekly,
  title: '주간 모의고사',
  plan: MockExamSessionRepository.weeklyDefaultPlan,
  startDescription: '주간 모의고사를 시작해 보세요.',
  orderDescription: '문제 순서는 듣기 10문제 후 독해 10문제로 고정됩니다.',
  insufficientDescription:
      '현재 트랙에서 주간 모의고사 문항이 충분하지 않습니다. 콘텐츠가 추가되면 이용할 수 있어요.',
);

const MockExamFlowConfig monthlyMockExamFlowConfig = MockExamFlowConfig(
  type: MockExamType.monthly,
  title: '월간 모의고사',
  plan: MockExamSessionRepository.monthlyDefaultPlan,
  startDescription: '월간 모의고사를 시작해 보세요.',
  orderDescription: '문제 순서는 듣기 17문제 후 독해 28문제로 고정됩니다.',
  insufficientDescription:
      '현재 트랙에서 월간 모의고사 문항이 충분하지 않습니다. 콘텐츠가 추가되면 이용할 수 있어요.',
);

class MockExamFlowScreen extends ConsumerStatefulWidget {
  const MockExamFlowScreen({
    super.key,
    required this.track,
  ...
  final String track;
  final MockExamFlowConfig config;
  final DateTime? nowLocal;

  @override
  ConsumerState<MockExamFlowScreen> createState() => _MockExamFlowScreenState();
}

class _MockExamFlowScreenState extends ConsumerState<MockExamFlowScreen> {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: AppSpacing.xs),
            Text(
              widget.config.insufficientDescription,
              style: AppTypography.body.copyWith(
                color: AppColors.textSecondary,
              ),
            ),
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          PrimaryPillButton(
            key: const ValueKey<String>('mock-open-result'),
            label: '결과 보기',
            onPressed: () {
